...
{
RIOFgReplay::RIOFgReplay(
    const std::string& config_filename,
    const std::map<std::string, std::string>& topic_names_and_topics,
    const std::string& input_bag, ros::NodeHandle& nh)
    : RIOFg(topic_names_and_topics.at("pc2"), topic_names_and_topics.at("imu"),
            topic_names_and_topics.at("pose"),
            topic_names_and_topics.at("state"), config_filename,
            Config::kWriteFeaturesToFile, nh),
...
void RIOFgReplay::writeState(const aaucns_rio::DoubleArrayStampedPtr& msg)
{
    output_bag_.write(topic_names_and_topics_.at("state"), msg->header.stamp,
                      msg);
};

void RIOFgReplay::writePose(
